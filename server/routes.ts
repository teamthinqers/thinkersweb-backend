...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // ===== MAPPING ENDPOINTS FOR DRAG-AND-DROP =====
  ...
  app.put(`${apiPrefix}/mapping/dot-to-wheel`, isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!dotId || !wheelId) {
        return res.status(400).json({ error: 'dotId and wheelId are required' });
      }
      
      // Update dot's wheelId (from dots table or entries table)
      const userId = req.user!.id;
      
      // Try updating in dots table first
      const dotsResult = await db.update(dots)
        .set({ wheelId: parseInt(wheelId) })
        .where(and(eq(dots.id, parseInt(dotId)), eq(dots.userId, userId)))
        .returning();
      
      if (dotsResult.length === 0) {
        // Try updating in entries table (fallback)
        const entriesResult = await db.update(entries)
          .set({ wheelId: parseInt(wheelId) })
          .where(and(eq(entries.id, dotId), eq(entries.userId, userId)))
          .returning();
          
        if (entriesResult.length === 0) {
          return res.status(404).json({ error: 'Dot not found or not owned by user' });
        }
      }
      
      res.json({ success: true, message: 'Dot mapped to wheel successfully' });
    } catch (error) {
  ...
  app.put(`${apiPrefix}/mapping/wheel-to-chakra`, isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      if (!wheelId || !chakraId) {
        return res.status(400).json({ error: 'wheelId and chakraId are required' });
      }
      
      const userId = req.user!.id;
      
      // Update wheel's chakraId
      const result = await db.update(wheels)
        .set({ chakraId: parseInt(chakraId) })
        .where(and(eq(wheels.id, parseInt(wheelId)), eq(wheels.userId, userId)))
        .returning();
      
      if (result.length === 0) {
        return res.status(404).json({ error: 'Wheel not found or not owned by user' });
      }
      
      res.json({ success: true, message: 'Wheel mapped to chakra successfully' });
    } catch (error) {
