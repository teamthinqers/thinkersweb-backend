...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get(`${apiPrefix}/neural-extension/status`, isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      ...
      console.error("Error getting neural extension status:", error);
      res.status(500).json({ error: 'Failed to get neural extension status' });
    }
  });
  
  // Get insights generated by the neural extension
  app.get(`${apiPrefix}/neural-extension/insights`, isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
      ...
      const { generateUserInsights } = await import('./neural-extension');
      const insights = await generateUserInsights(userId);
      
      res.status(200).json({ insights });
    } catch (error) {
      console.error("Error generating neural extension insights:", error);
      res.status(500).json({ error: 'Failed to generate insights' });
    }
  });
  
  // Get topic recommendations based on user history
  app.get(`${apiPrefix}/neural-extension/recommendations`, isAuthenticated, async (req: AuthenticatedRequest, res: Response) => {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'User not authenticated' });
      ...
      const { getRecommendedTopics } = await import('./neural-extension');
