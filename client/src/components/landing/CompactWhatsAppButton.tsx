...
const CompactWhatsAppButton: React.FC = () => {
  ...
  return (
    <Button
      ...
      className="bg-gradient-to-r from-[#25D366] to-[#128C7E] hover:from-[#128C7E] hover:to-[#25D366] text-white border-0 flex items-center gap-1 px-3 py-1 h-8 shadow-md hover:scale-[1.05] transition-all duration-200 rounded-md"
      size="sm"
      disabled={isLoading}
    >
      {isLoading ? (
        ...
        <>
          ...
          <svg className="w-5 h-5 animate-pulse-slow" viewBox="0 0 24 24" fill="white">
          ...
          <span className="text-xs font-medium">DotSpark</span>
        </>
      )}
    </Button>
  );
