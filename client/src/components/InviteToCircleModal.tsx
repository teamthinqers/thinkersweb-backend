...
export function InviteToCircleModal({ open, onOpenChange, circleId, circleName }: InviteToCircleModalProps) {
  ...
  const sendInvitesMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest('POST', `/api/thinq-circles/${circleId}/invite`, {
    ...
    onSuccess: () => {
      toast({
        title: 'Invites sent!',
        description: `Invitations sent to ${selectedUsers.length + emailInvites.length} people.`,
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-2xl">Invite to {circleName}</DialogTitle>
          <DialogDescription>
            Invite people to collaborate in this ThinQ Circle
          </DialogDescription>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button 
            ...
            disabled={sendInvitesMutation.isPending || (selectedUsers.length === 0 && emailInvites.length === 0)}
            className="bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-700 hover:to-violet-700"
          >
            {sendInvitesMutation.isPending ? 'Sending...' : `Send ${selectedUsers.length + emailInvites.length || ''} Invite${(selectedUsers.length + emailInvites.length) !== 1 ? 's' : ''}`}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
