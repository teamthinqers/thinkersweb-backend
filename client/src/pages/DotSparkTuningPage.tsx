    ...
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      ...
      <Card className="mb-8 bg-gradient-to-br from-purple-50 to-white dark:from-purple-950/30 dark:to-gray-950 border border-purple-100 dark:border-purple-900/50 overflow-hidden">
        ...
        <CardHeader className="pb-2 relative z-10">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <img src="/dotspark-logo-wordmark.png?v=2" alt="DotSpark" className="h-5 w-auto rounded-sm" />
              <CardTitle>
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          <h2 className="text-lg font-semibold">DotSpark Configuration</h2>
          <span className="text-sm text-muted-foreground">
            Step {activeTab === 'hygiene' ? '1' : 
                 activeTab === 'memory' ? '2' : 
                 activeTab === 'learning' ? '3' : 
                 activeTab === 'expertise' ? '4' : '1'} of 4
          </span>
        </div>
        <div className="w-full h-2 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden mb-2">
          <div className="h-full bg-gradient-to-r from-violet-500 via-blue-500 to-emerald-500 rounded-full transition-all duration-500"
            style={{ 
              width: `${activeTab === 'hygiene' ? 25 : 
                      activeTab === 'memory' ? 50 : 
                      activeTab === 'learning' ? 75 : 
                      activeTab === 'expertise' ? 100 : 25}%` 
            }}
          />
        </div>
        <div className="flex justify-between text-xs text-muted-foreground">
          <div className={`flex flex-col items-center ${activeTab === 'hygiene' ? 'text-violet-600 dark:text-violet-400 font-medium' : ''}`}>
            <div className={`w-2 h-2 rounded-full mb-1 ${activeTab === 'hygiene' ? 'bg-violet-500' : 'bg-gray-300 dark:bg-gray-700'}`}></div>
            <span>Cognitive</span>
          </div>
          <div className={`flex flex-col items-center ${activeTab === 'memory' ? 'text-blue-600 dark:text-blue-400 font-medium' : ''}`}>
            <div className={`w-2 h-2 rounded-full mb-1 ${activeTab === 'memory' ? 'bg-blue-500' : 'bg-gray-300 dark:bg-gray-700'}`}></div>
            <span>Memory</span>
          </div>
          <div className={`flex flex-col items-center ${activeTab === 'learning' ? 'text-emerald-600 dark:text-emerald-400 font-medium' : ''}`}>
            <div className={`w-2 h-2 rounded-full mb-1 ${activeTab === 'learning' ? 'bg-emerald-500' : 'bg-gray-300 dark:bg-gray-700'}`}></div>
            <span>Learning</span>
          </div>
          <div className={`flex flex-col items-center ${activeTab === 'expertise' ? 'text-amber-600 dark:text-amber-400 font-medium' : ''}`}>
            <div className={`w-2 h-2 rounded-full mb-1 ${activeTab === 'expertise' ? 'bg-amber-500' : 'bg-gray-300 dark:bg-gray-700'}`}></div>
            <span>Expertise</span>
          </div>
        </div>
      </div>
      ...
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between pb-2">
          <div>
            <CardTitle>
              {activeTab === 'hygiene' && 'Cognitive Processing'}
              {activeTab === 'memory' && 'Memory Configuration'}
              {activeTab === 'learning' && 'Learning Focus'}
              {activeTab === 'expertise' && 'Expertise Domains'}
            </CardTitle>
            <CardDescription>
              {activeTab === 'hygiene' && 'Configure how your DotSpark processes information'}
              {activeTab === 'memory' && 'Define how your DotSpark stores and recalls information'}
              {activeTab === 'learning' && 'Set priorities for what your DotSpark should learn'}
              {activeTab === 'expertise' && 'Customize knowledge domains your DotSpark specializes in'}
            </CardDescription>
          </div>
          <div className="flex space-x-1">
            <Button 
              ...
              variant="outline" 
              onClick={() => {
                const tabs = ['hygiene', 'memory', 'learning', 'expertise'];
                const currentIndex = tabs.indexOf(activeTab);
                if (currentIndex > 0) {
                  setActiveTab(tabs[currentIndex - 1]);
                }
              }}
              disabled={activeTab === 'hygiene'}
              className={activeTab === 'hygiene' ? 'opacity-50 cursor-not-allowed' : ''}
            >
            ...
            <Button 
              ...
              disabled={activeTab === 'expertise'}
              className={activeTab === 'expertise' ? 'opacity-50 cursor-not-allowed' : ''}
            >
              Next
        ...
        <CardContent>
          <div className="w-full">
            {/* Modified tabs to add memory section */}
            <div className="hidden">
              <button onClick={() => setActiveTab('hygiene')}>Cognitive</button>
              <button onClick={() => setActiveTab('memory')}>Memory</button>
              <button onClick={() => setActiveTab('learning')}>Learning</button>
              <button onClick={() => setActiveTab('expertise')}>Expertise</button>
            </div>
