...
const Dashboard: React.FC = () => {
  ...
  const DotWheelsMap: React.FC<{ 
    ...
    useEffect(() => {
      if (!user) {
        setPreviewMode(true); // Automatically show preview mode for unsigned users
      }
    }, [user]);
    ...
    if (!previewMode && (!user || userWheels.length === 0 && dots.length === 0)) {
      ...
      if (!user) {
        // For unauthenticated users, automatically switch to preview mode
        setPreviewMode(true);
        return null; // This will trigger a re-render with preview mode
      }
      
      // For authenticated users with no content, show empty state with creation options
      return (
        <div className="relative bg-gradient-to-br from-amber-50/50 to-orange-50/50 rounded-xl p-4 min-h-[500px] border-2 border-amber-200 shadow-lg overflow-hidden">
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-amber-800 mb-2">Start Creating Content</h3>
              <p className="text-amber-700 mb-6 max-w-md mx-auto">
                Create your first dots, wheels, and chakras to organize your thoughts.
              </p>
              
              <div className="flex flex-col gap-3 max-w-xs mx-auto">
                <p className="text-xs text-amber-600">
                  Try Preview Mode to see examples, or create your own content above
                </p>
              </div>
            </div>
        </div>
