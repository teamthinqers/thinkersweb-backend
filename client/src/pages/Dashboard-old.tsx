...
const Dashboard: React.FC = () => {
  ...
  const performSearch = (query: string) => {
    ...
    // Use current data source - dots and wheels are already loaded from correct endpoints
    const searchDots = dots;
    const searchWheels = userWheels;
  ...
  const DataDisplayComponent = () => {
    // Use data from gridData API calls which already route to correct endpoints
    const actualDots = gridData?.data ? Object.keys(gridData.data.dotPositions).map(dotId => {
      return dots.find(d => d.id === dotId);
    }).filter(Boolean) as Dot[] : dots;
    
    // Base wheels to display - data already comes from correct endpoints based on mode
    ...
    return (
      <div className="space-y-6">
        ...
        <DotWheelsMapWrapper 
          ...
          displayWheels={displayWheels}
          totalDots={totalDots}
          totalWheels={totalWheels}
          totalChakras={totalChakras}
    ...
    const displayWheels = userWheels; // Now using data from correct endpoints based on mode
    
    // Real mode shows ONLY user data, Preview mode shows ONLY demo data
    let baseDotsToDisplay = dots; // Now using data from correct endpoints based on mode
